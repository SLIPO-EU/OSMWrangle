...
public class Mapping {
	...
	public class mapProperties {
		...
		public void setMappingProfile() {
			...
			if ((resourceType != null) && (instance != null) && (part == null))
			{   //Handling of properties that are instances of classes, e.g., CONTACT (phone, fax, email), NAME, ACCURACY in the SLIPO ontology 
				if (language != null)
			...
			{	//Specific handling of composite properties (e.g., like ADDRESS consisting of street name, house number, postal code, etc.)
				if (language != null)
			...
			{   //Specific handling of literals with data type specifications
	...
	public void add(String key, String[] mappings) {
		...
		{
			switch (i) {
			   case 0: props.setEntityType(mappings[i]); break;
			   case 1:  props.setPredicate(mappings[i]); break;
			   case 2:  props.setResourceType(mappings[i]); break;	 
			...
			try {
				...
				for (String key : map.keySet()) 
				{
					...
					for (String subkey : map.get(key).keySet()) {
						switch (subkey) {
						   case "entity": props.setEntityType(map.get(key).get(subkey)); break;
						   case "predicate":  props.setPredicate(map.get(key).get(subkey)); break;
						   case "type":  props.setResourceType(map.get(key).get(subkey)); break;	 
				...
				ExceptionHandler.abort(e, "ERROR: Cannot load YML file to a particular mapping.");
			}
	    }
	    catch(Exception e) {
	    	ExceptionHandler.abort(e, "ERROR: Cannot parse the specified YML file with attribute mappings.");	    	
	    }
	}
